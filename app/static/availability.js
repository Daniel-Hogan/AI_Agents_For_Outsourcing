...
function initAvailabilityCalendar() {
  ...
  roots.forEach((root) => {
    ...
    form?.addEventListener("submit", (event) => {
      event.preventDefault();
      if (isSaving || setsEqual(cellSet, savedCellSet)) {
        updateSaveButtonState();
        return;
      }
      ...
      fetch(form.action || '/availability/add', {
        method: 'POST',
        body: formData,
        ...
        .then((response) => {
          ...
          savedCellSet = new Set(cellSet);
          root.dataset.availabilitySelectedCells = serializeCells(savedCellSet);
        })
        .catch((err) => {
